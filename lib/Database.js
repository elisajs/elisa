...
export default class Database {
  ...
  getCollection(...args) {
    ...
    //(1) arguments
    [ns, ds, opts] = parseDataStoreArgs(args);

    //(2) get datastore
    if (ns) ds = this.getNamespace(ns, opts).getCollection(ds, opts);
    else ds = new (this.connection.getCollectionClass())(this, ds, opts);
   ...
   */
  hasCollection(...args) {
    var ns, ds, opts, callback, res;

    //(1) pre: arguments
    [ns, ds, opts, callback] = parseDataStoreArgs(args);

    //(2) check
    if (ns) {
      return this.getNamespace(ns).hasCollection(ds, callback);
    } else {
      if (this.sync) {
        return !!this.findCollection(ds);
      } else {
        this.findCollection(ds, (error, ds) => {
          if (error) callback(error);
          else callback(undefined, !!ds);
        });
      }
    }
  }
  ...
  findCollection(...args) {
    ...
    //(2) read
    if (ns) {
      return this.getNamespace(ns, opts).findCollection(ds, opts, callback);
    } else {
      if (this.sync) return sync((done) => this._findCollection(ds, opts, done));
      else this._findCollection(ds, opts, callback);
    }
  }
